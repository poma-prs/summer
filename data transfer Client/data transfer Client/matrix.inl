...
inline void CMatrix3X3::FromAxisAngle(CVector3& vec, float fAngle)
{
	...
	m_fMat[0] = fCos + SQU(vec.x) * fOMC;
	m_fMat[4] = fCos + SQU(vec.y) * fOMC;
	m_fMat[8] = fCos + SQU(vec.z) * fOMC;

	m_fMat[1] = vec.x * vec.y * fOMC + vec.z * fSin;
	m_fMat[3] = vec.x * vec.y * fOMC - vec.z * fSin;
	m_fMat[2] = vec.x * vec.z * fOMC + vec.y * fSin;
	m_fMat[6] = vec.x * vec.z * fOMC - vec.y * fSin;
	m_fMat[5] = vec.x * vec.z * fOMC + vec.x * fSin;
	m_fMat[7] = vec.x * vec.z * fOMC - vec.x * fSin;
...
inline void CMatrix4X4::FromAxisAngle(CVector3& vec, float fAngle)
{
	...
	m_fMat[0] = fCos + SQU(vec.x) * fOMC;
	m_fMat[5] = fCos + SQU(vec.y) * fOMC;
	m_fMat[10] = fCos + SQU(vec.z) * fOMC;
	m_fMat[15] = 1.0f;
	m_fMat[4] = vec.x * vec.y * fOMC + vec.z * fSin;
	m_fMat[1] = vec.x * vec.y * fOMC - vec.z * fSin;
	m_fMat[8] = vec.x * vec.z * fOMC + vec.y * fSin;
	m_fMat[2] = vec.x * vec.z * fOMC - vec.y * fSin;
	m_fMat[9] = vec.y * vec.z * fOMC + vec.x * fSin;
	m_fMat[6] = vec.y * vec.z * fOMC - vec.x * fSin;
