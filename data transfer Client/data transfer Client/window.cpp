					 ...
					 unsigned int uiBpp, unsigned int uiFlags)
{
	...
	if(m_uiFlags == FLAG_CHOICE)
	{
		DialogBox(m_hInstance, MAKEINTRESOURCE(IDD_SETTINGS), NULL, DlgProc);
	}
...
void CWindow::Resize(unsigned int uiWidth, unsigned int uiHeight)
{
	...
	gluPerspective(45.0f,(float)uiWidth/(float)uiHeight,0.1f,2000.0f);
			...
			case IDOK:
				{
					...
					if (IsDlgButtonChecked(hWnd, IDC_CONNECT))
					{
						APP->GetSet()->network = true;
						GetDlgItemText(hWnd, IDC_IPADDRESS, APP->GetSet()->ip_server, 100);
					}
					else
					{
						APP->GetSet()->network = false;
					}
