...
inline void CQuaternion::FromAxisAngle(CVector3& rAxis, float fAngle)
{
	...
	m_fQuat[0] = rAxis.x * sa;
	m_fQuat[1] = rAxis.y * sa;
	m_fQuat[2] = rAxis.z * sa;
	m_fQuat[3] = ca;
}
...
{
	float dSY = sin(fZ * 0.5f);
	float dSP = sin(fY * 0.5f);
	float dSR = sin(fX * 0.5f);
	float dCY = cos(fZ * 0.5f);
	float dCP = cos(fY * 0.5f);
	float dCR = cos(fX * 0.5f);
...
{
	float m11 = SQU(m_fQuat[3]) + SQU(m_fQuat[0]) - SQU(m_fQuat[1]) - SQU(m_fQuat[2]);
	float m21 = 2 * (m_fQuat[0] * m_fQuat[1] + m_fQuat[2] * m_fQuat[3]);
	float m31 = 2 * (m_fQuat[2] * m_fQuat[0] - m_fQuat[1] * m_fQuat[3]);
	float m32 = 2 * (m_fQuat[2] * m_fQuat[1] + m_fQuat[0] * m_fQuat[3]);
	float m33 = SQU(m_fQuat[3]) - SQU(m_fQuat[0]) - SQU(m_fQuat[1]) + SQU(m_fQuat[2]);
...
{
	return CQuaternion(-(m_fQuat[0] * rVec.x + m_fQuat[1] * rVec.y + m_fQuat[2] * rVec.z), 
                         m_fQuat[3] * rVec.x + m_fQuat[1] * rVec.z + m_fQuat[2] * rVec.y, 
                         m_fQuat[3] * rVec.y + m_fQuat[2] * rVec.x + m_fQuat[0] * rVec.z,
                         m_fQuat[3] * rVec.z + m_fQuat[0] * rVec.y + m_fQuat[1] * rVec.x ); 
}
...
{
	CQuaternion tmp(m_fQuat[3] * rVec.x + 
						m_fQuat[1] * rVec.z - m_fQuat[2] * rVec.y,

					   m_fQuat[3] * rVec.y + 
						m_fQuat[2] * rVec.x - m_fQuat[0] * rVec.z,
						
					   m_fQuat[3] * rVec.z + 
						m_fQuat[0] * rVec.y - m_fQuat[1] * rVec.x,
					   
					   -(m_fQuat[0] * rVec.x + 
						m_fQuat[1] * rVec.y + m_fQuat[2] * rVec.z));
	*this = tmp;
}
